...
// this is main api of all category 
...
const takeCatagoryName = async () => {
    const catatoryName = await getCatagoryName();
    const navList = document.getElementById('nav-list');

    catatoryName.forEach(element => {
...
const News = async(allNewsInfo) => {
    const totalNewsAmount = allNewsInfo.data.length;
    const totalNews = document.getElementById('total-news');
    totalNews.innerHTML ='';
    const addItems = document.getElementById('add-items');
    addItems.innerHTML = '';
    totalNews.innerText = totalNewsAmount + ' ' + `items found for ${categoryname? categoryname : 'All News'}`;
